...
function fillEntryArray(size) {
  const entries = [];
  for (let i = 0; i < size; i++) {
    entries.push(createEntry(Math.floor(Math.random() * 10) + 1));
  }
  return entries;
}

function addDictsTogether(size, numTimes) {
  const arr1 = fillEntryArray(size);
  const arr2 = fillEntryArray(size);

  let duration = 0;
  for (let num = 0; num < numTimes + 1; num++) {
    let result = 0;
  ...
  return duration / numTimes;
}

function addArraysTogether(size, padding, numTimes) {
  const arr1 = createArrays(1, padding, size);
  const arr2 = createArrays(2, padding, size);
...
function main() {
  ...
  console.log(`Running with dict size: ${sizeof(createEntry(1))} bytes`);

  for (let n = 1; n <= numIterations; n++) {
    const N = 30700 * n;
    const duration = addDictsTogether(N, 30);
    console.log(`  N = ${N}: ${duration * 1000} microseconds.`);
  }

  const array_size = 256;
  console.log(`Running with array size: ${sizeof(createArrays(array_size, array_size)[0])} bytes`);
  for (let n = 1; n <= numIterations; n++) {
    const N = 30700 * n;
    const duration = addArraysTogether(N, array_size, 30);
    console.log(`  N = ${N}: ${duration * 1000} microseconds.`);
  }
}
